...
const BlogAdmin: React.FC = () => {
  const [articles, setArticles] = useState<BlogArticleData[]>(Object.values(BLOG_REGISTRY));
  const [activeArticle, setActiveArticle] = useState<BlogArticleData>(articles[0]);
  const [isEditMode, setIsEditMode] = useState(true);

  // Form states matching active article
  const [title, setTitle] = useState(activeArticle.title);
  const [category, setCategory] = useState(activeArticle.category);
  const [excerpt, setExcerpt] = useState(activeArticle.excerpt);
  ...
  const handleSave = () => {
    setArticles((prev: BlogArticleData[]) => prev.map((a: BlogArticleData) => a.slug === activeArticle.slug ? {
      ...a,
      title,
      category,
      excerpt,
  ...
  const handleCreateNew = () => {
    ...
    setArticles((prev: BlogArticleData[]) => [...prev, newArt]);
